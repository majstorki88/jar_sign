...
ECHO WARNING:
ECHO.
ECHO Self-signing Jar files is no longer recommended.
ECHO Jar files should be digitally signed using a trusted certificate.
ECHO.
...
SET KEYSTORE_PASSWORD=telekomad
REM
...
REM
REM *** Remove the text after the '=' below and replace with your password. ***
SET JAR_KEY_PASSWORD=telekomad
REM
...
D:\Oracle\frs122\oracle_common\jdk\bin\keytool -genkey -dname "CN=%DN_CN%, OU=%DN_OU%, O=%DN_O%, C=%DN_C%" -alias %JAR_KEY% -keypass %JAR_KEY_PASSWORD% -keystore %KEYSTORE% -storepass %KEYSTORE_PASSWORD% -validity %VALIDDAYS%
...
D:\Oracle\frs122\oracle_common\jdk\bin\jarsigner -keystore %KEYSTORE% -storepass %KEYSTORE_PASSWORD% -keypass %JAR_KEY_PASSWORD% %1 %JAR_KEY%
REM Check for any error
